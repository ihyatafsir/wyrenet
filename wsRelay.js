...
const server = http.createServer((req, res) => {
    if (req.method === 'GET') {
        const htmlPath = path.join(__dirname, 'p2p-client.html');
    ...
    ws.on('message', (data) => {
        try {
            ...
            switch (msg.type) {
                ...
                case 'MSG':
                    const displayPreview = msg.content || (msg.zbat ? '[ZBAT Encrypted]' : '');
                    console.log(`[MSG] ${peerId} → ${msg.to || 'all'}: ${displayPreview.slice(0, 50)}`);
                    ...
                    if (msg.to && msg.to !== 'all') {
                        let targetPeer = peers.get(msg.to);
                        if (!targetPeer) {
                            const prefix = msg.to.split('@')[0];
                        ...
                        if (targetPeer && targetPeer.ws.readyState === WebSocket.OPEN) {
                            targetPeer.ws.send(JSON.stringify(forwardPacket));
                            console.log(`[RELAY] Delivered to ${targetPeer.identity || msg.to}`);
                        } else {
                            broadcast(forwardPacket, peerId);
                        }
                    } else {
                        broadcast(forwardPacket, peerId);
                    }
