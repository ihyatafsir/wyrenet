...
async function testMiftahEncryption(): Promise<TestResult[]> {
    ...
    const alicePrivate = ed.utils.randomSecretKey();
    const bobPublic = ed.utils.randomSecretKey(); // Just need random bytes for test
...
async function testBarqProtocol(): Promise<TestResult[]> {
    ...
    const alicePrivate = ed.utils.randomSecretKey();
    const bobPublic = ed.utils.randomSecretKey();
