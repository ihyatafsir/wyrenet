...
class ShabahStego extends EventTarget {
     ...
     */
    hideInText(coverText, hiddenMessage) {
     ...
     */
    encodeAsEmoji(data) {
        const bytes = buffer_1.Buffer.from(data, 'utf8');
        const emojis = [];
     ...
     */
    hideInImage(imageData, hiddenMessage) {
     ...
     */
    encodePeerAsColors(peerId, publicKeyHex) {
    ...
    decodePeerFromColors(colors) {
        try {
            ...
            const data = buffer_1.Buffer.from(bytes).toString('utf8').replace(/\0+$/, '');
     ...
     */
    createInvisibleMessage(message) {
     ...
     */
    readInvisibleMessage(invisible) {
        const zwChars = [];
        const zeroWidthValues = Object.values(ZERO_WIDTH);
     ...
     */
    calculateCapacity(carrier, method) {
        switch (method) {
            case 'waswas':
...
// Singleton export
exports.shabahStego = new ShabahStego();
exports.default = ShabahStego;
