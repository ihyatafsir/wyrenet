 ...
 * ├─────────────────────────────────────────────────────────────────┤
 * │                       Stealth Layer                             │
 * │                    Maladh (مَلَاذ) — Phase 10                   │
 * │  Hajar + Ramad + Qina' + Dalil + Ghayba + Shabah               │
 * └─────────────────────────────────────────────────────────────────┘
 * 
 * ═══════════════════════════════════════════════════════════════════
 */
...
import { maladhDiscovery, MaladhBeacon } from './MaladhDiscovery';
...
export const LISAN_DIAGNOSTICS = {
    ...
    // Stealth states (مَلَاذ)
    MALADH_TAKHAFFI: { ar: 'مَلَاذ تَخَفِّي', meaning: 'Stealth mode activated' },
    MALADH_KASHF: { ar: 'مَلَاذ كَشْف', meaning: 'Stealth peer discovered' },
    MALADH_TAWSIL: { ar: 'مَلَاذ تَوْصِيل', meaning: 'Connecting to stealth-discovered peer' },
};
...
class Munassiq extends EventEmitter {
    ...
    async tamhid(identityPrefix: string): Promise<WyreSUpIdentity> {
        ...
        // Wire Maladh stealth discovery to auto-connect discovered peers
        maladhDiscovery.addListener('peerDiscovered', (beacon: MaladhBeacon) => {
            console.log(`[MUNASSIQ] [مَلَاذ كَشْف] Stealth peer discovered: ${beacon.peerId.slice(0, 8)}...`);
            this.emitDiagnostic('MALADH_KASHF', `Stealth peer: ${beacon.peerId.slice(0, 16)}`);
            this.emit('stealth-peer-discovered', beacon);
        });

        this.initialized = true;
        this.emit('initialized', this.identity);

        console.log(`[MUNASSIQ] ✓ Protocol stack ready (with مَلَاذ stealth layer)`);
    ...
    // ═══════════════════════════════════════════════════════════════
    // مَلَاذ (Maladh) — Stealth Discovery Control
    // From: "لَاذَ بالشَّيء" - To seek refuge, shelter beside something
    // ═══════════════════════════════════════════════════════════════

    /**
     * تَخَفِّي (Takhaffi) - Toggle stealth discovery mode
     * From: "تَخَفَّى" - To conceal oneself, become hidden
     */
    takhaffi(enable: boolean): void {
        if (enable) {
            maladhDiscovery.activate();
            this.emitDiagnostic('MALADH_TAKHAFFI', 'Stealth discovery ACTIVATED');
        } else {
            maladhDiscovery.deactivate();
            this.emitDiagnostic('MALADH_TAKHAFFI', 'Stealth discovery DEACTIVATED');
        }
        this.emit('stealth-mode-changed', enable);
    }

    /**
     * تَوْصِيل خَفِيّ (Tawsil Khafiyy) - Connect to stealth-discovered peer
     */
    async tawsilKhafiyy(beacon: MaladhBeacon): Promise<boolean> {
        this.emitDiagnostic('MALADH_TAWSIL', `Connecting to stealth peer: ${beacon.peerId.slice(0, 16)}`);

        // Convert beacon public key to Uint8Array for Tawsil
        const pubKeyBytes = new TextEncoder().encode(beacon.publicKey);

        // Use primary endpoint from beacon
        const endpoint = beacon.endpoints[0] ? {
            ip: beacon.endpoints[0].host,
            port: beacon.endpoints[0].port,
        } : undefined;

        return this.tawsil(beacon.peerId, pubKeyBytes, endpoint);
    }
