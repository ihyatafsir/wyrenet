...
exports.SovereignMeshConsensus = exports.ConsensusStateTransitionError = exports.QuorumEvaluationError = exports.BlockHeaderValidationError = exports.ConsensusPreconditionError = exports.MeshRoutingCycleError = exports.SignatureVerificationError = exports.EpochDivergenceError = exports.MeshHopLimitExceededError = exports.ConsensusDeadlockViolationError = exports.SovereignMeshError = exports.ConsensusEpochState = void 0;
const node_crypto_1 = require("node:crypto");
const node_buffer_1 = require("node:buffer");
var ConsensusEpochState;
(function (ConsensusEpochState) {
    ConsensusEpochState["INITIALIZING"] = "INITIALIZING";
    ConsensusEpochState["PROPOSING"] = "PROPOSING";
    ConsensusEpochState["VOTING"] = "VOTING";
    ConsensusEpochState["COMMITTED"] = "COMMITTED";
    ConsensusEpochState["QUORUM_REJECTED"] = "QUORUM_REJECTED";
})(ConsensusEpochState || (exports.ConsensusEpochState = ConsensusEpochState = {}));
// ============================================================================
// SECTION 2: Ibn Manzur Error Taxonomy
// ============================================================================
class SovereignMeshError extends Error {
    constructor(errorCode, diagnosticMessage, causalContext) {
...
exports.SovereignMeshError = SovereignMeshError;
class ConsensusDeadlockViolationError extends SovereignMeshError {
    constructor(diagnosticMessage, causalContext) {
        super("CONSENSUS_DEADLOCK_VIOLATION", diagnosticMessage, causalContext);
        this.name = "ConsensusDeadlockViolationError";
    }
}
exports.ConsensusDeadlockViolationError = ConsensusDeadlockViolationError;
class MeshHopLimitExceededError extends SovereignMeshError {
    constructor(diagnosticMessage, causalContext) {
        super("MESH_HOP_LIMIT_EXCEEDED", diagnosticMessage, causalContext);
        this.name = "MeshHopLimitExceededError";
    }
}
exports.MeshHopLimitExceededError = MeshHopLimitExceededError;
class EpochDivergenceError extends SovereignMeshError {
    constructor(diagnosticMessage, causalContext) {
        super("EPOCH_DIVERGENCE", diagnosticMessage, causalContext);
        this.name = "EpochDivergenceError";
    }
}
exports.EpochDivergenceError = EpochDivergenceError;
class SignatureVerificationError extends SovereignMeshError {
    constructor(diagnosticMessage, causalContext) {
...
class MeshRoutingCycleError extends SovereignMeshError {
    constructor(diagnosticMessage, causalContext) {
        super("MESH_ROUTING_CYCLE", diagnosticMessage, causalContext);
        this.name = "MeshRoutingCycleError";
    }
}
exports.MeshRoutingCycleError = MeshRoutingCycleError;
class ConsensusPreconditionError extends SovereignMeshError {
    constructor(diagnosticMessage, causalContext) {
        super("CONSENSUS_PRECONDITION_VIOLATION", diagnosticMessage, causalContext);
        this.name = "ConsensusPreconditionError";
    }
}
exports.ConsensusPreconditionError = ConsensusPreconditionError;
class BlockHeaderValidationError extends SovereignMeshError {
    constructor(diagnosticMessage, causalContext) {
        super("BLOCK_HEADER_VALIDATION_FAILED", diagnosticMessage, causalContext);
        this.name = "BlockHeaderValidationError";
    }
}
exports.BlockHeaderValidationError = BlockHeaderValidationError;
class QuorumEvaluationError extends SovereignMeshError {
    constructor(diagnosticMessage, causalContext) {
        super("QUORUM_EVALUATION_FAILED", diagnosticMessage, causalContext);
        this.name = "QuorumEvaluationError";
    }
}
exports.QuorumEvaluationError = QuorumEvaluationError;
class ConsensusStateTransitionError extends SovereignMeshError {
    constructor(diagnosticMessage, causalContext) {
        super("CONSENSUS_STATE_TRANSITION_FAILED", diagnosticMessage, causalContext);
        this.name = "ConsensusStateTransitionError";
    }
}
exports.ConsensusStateTransitionError = ConsensusStateTransitionError;
// ============================================================================
// SECTION 3: Bounded LRU Deduplication Cache (Daf' al-Tasalsul)
// ============================================================================
class BoundedLruDeduplicationCache {
    constructor(maximumEntryCapacity) {
        if (!Number.isInteger(maximumEntryCapacity) || maximumEntryCapacity <= 0) {
            throw new ConsensusPreconditionError("BoundedLruDeduplicationCache requires a positive integer capacity", { providedCapacity: maximumEntryCapacity });
        }
        this.maximumEntryCapacity = maximumEntryCapacity;
        this.insertionOrderedKeys = [];
        this.envelopeIdentifierSet = new Set();
    }
    hasSeenEnvelope(envelopeIdentifier) {
    ...
    recordEnvelope(envelopeIdentifier) {
        if (this.envelopeIdentifierSet.has(envelopeIdentifier)) {
            const existingIndex = this.insertionOrderedKeys.indexOf(envelopeIdentifier);
            if (existingIndex >= 0) {
                this.insertionOrderedKeys.splice(existingIndex, 1);
...
class MonotonicPeerLockRegistry {
    constructor() {
        this.heldLockKeyHexSet = new Set();
        this.lockAcquisitionOrder = [];
    }
    acquireLocksMonotonically(peerPublicKeyHexes) {
        if (peerPublicKeyHexes.length === 0) {
            return;
        }
        const sortedUniqueKeys = Array.from(new Set(peerPublicKeyHexes)).sort((leftKey, rightKey) => leftKey < rightKey ? -1 : leftKey > rightKey ? 1 : 0);
        for (const candidateKeyHex of sortedUniqueKeys) {
            if (this.heldLockKeyHexSet.has(candidateKeyHex)) {
                throw new ConsensusDeadlockViolationError("Attempted to re-acquire an already held peer lock", { conflictingPeerKeyHex: candidateKeyHex });
            }
        }
        for (let index = 0; index < sortedUniqueKeys.length; index += 1) {
            ...
            if (index > 0) {
                ...
                if (previousKeyHex >= currentKeyHex) {
                    throw new ConsensusDeadlockViolationError("Peer lock acquisition order is not strictly monotonic", { previousPeerKeyHex: previousKeyHex, currentPeerKeyHex: currentKeyHex });
                }
            }
    ...
    releaseLocksMonotonically(peerPublicKeyHexes) {
        const sortedUniqueKeys = Array.from(new Set(peerPublicKeyHexes)).sort((leftKey, rightKey) => leftKey < rightKey ? -1 : leftKey > rightKey ? 1 : 0);
        for (let index = sortedUniqueKeys.length - 1; index >= 0; index -= 1) {
            const currentKeyHex = sortedUniqueKeys[index];
            if (!this.heldLockKeyHexSet.has(currentKeyHex)) {
                throw new ConsensusDeadlockViolationError("Attempted to release a peer lock that is not held", { missingPeerKeyHex: currentKeyHex });
            }
...
class DirectedAcyclicMeshRoutingTable {
    constructor() {
        this.adjacencyByPeerKeyHex = new Map();
    }
    registerPeerNode(peerPublicKeyHex) {
        if (!this.adjacencyByPeerKeyHex.has(peerPublicKeyHex)) {
            this.adjacencyByPeerKeyHex.set(peerPublicKeyHex, new Set());
        }
    }
    registerDirectedEdge(fromPeerKeyHex, toPeerKeyHex) {
        if (fromPeerKeyHex === toPeerKeyHex) {
            throw new MeshRoutingCycleError("Self-loop edges are forbidden in the mesh DAG", { peerKeyHex: fromPeerKeyHex });
        }
        ...
        if (outgoingSet === undefined) {
            throw new MeshRoutingCycleError("Adjacency set missing after registration", { fromPeerKeyHex });
        }
        outgoingSet.add(toPeerKeyHex);
        if (this.wouldIntroduceCycle(fromPeerKeyHex, toPeerKeyHex)) {
            outgoingSet.delete(toPeerKeyHex);
            throw new MeshRoutingCycleError("Edge would introduce a cycle in the mesh DAG", { fromPeerKeyHex, toPeerKeyHex });
        }
    }
    wouldIntroduceCycle(fromPeerKeyHex, toPeerKeyHex) {
        const visitedPeerKeyHexSet = new Set();
        const pendingPeerKeyHexStack = [toPeerKeyHex];
    ...
    outgoingNeighborsOf(peerPublicKeyHex) {
        const outgoingSet = this.adjacencyByPeerKeyHex.get(peerPublicKeyHex);
        if (outgoingSet === undefined) {
            return [];
        }
        return Array.from(outgoingSet).sort((leftKey, rightKey) => leftKey < rightKey ? -1 : leftKey > rightKey ? 1 : 0);
    }
    hasPeerNode(peerPublicKeyHex) {
...
// ============================================================================
function computeDeterministicPathHashHex(traversedPeerKeyHexes) {
...
class MerkleRootCalculator {
    static isLowercaseHexDigest(candidateHex) {
    ...
    static computeMerkleRootHex(leafHashHexValues) {
        if (leafHashHexValues.length === 0) {
            throw new ConsensusPreconditionError("Cannot compute a Merkle root over an empty leaf set", { leafCount: leafHashHexValues.length });
        }
        for (let leafIndex = 0; leafIndex < leafHashHexValues.length; leafIndex += 1) {
            ...
            if (!MerkleRootCalculator.isLowercaseHexDigest(leafHashHex)) {
                throw new ConsensusPreconditionError("Merkle leaf must be a lowercase 64-character hexadecimal SHA-256 digest", { leafIndex, leafHashHex });
            }
        }
        let currentLevelHashHexValues = leafHashHexValues.slice();
        while (currentLevelHashHexValues.length > 1) {
            const nextLevelHashHexValues = [];
            for (let pairStartIndex = 0; pairStartIndex < currentLevelHashHexValues.length; pairStartIndex += 2) {
                const leftHashHex = currentLevelHashHexValues[pairStartIndex];
                const rightHashHex = pairStartIndex + 1 < currentLevelHashHexValues.length
                    ? currentLevelHashHexValues[pairStartIndex + 1]
                    : leftHashHex;
                const concatenatedBytes = node_buffer_1.Buffer.concat([
                    node_buffer_1.Buffer.from(leftHashHex, "hex"),
                    node_buffer_1.Buffer.from(rightHashHex, "hex"),
                ]);
                const parentDigestBytes = (0, node_crypto_1.createHash)("sha256").update(concatenatedBytes).digest();
...
class SovereignMeshConsensus {
    constructor(configuration) {
        if (!Number.isInteger(configuration.maximumGossipHopTtl) || configuration.maximumGossipHopTtl <= 0) {
            throw new ConsensusPreconditionError("maximumGossipHopTtl must be a positive integer", { providedValue: configuration.maximumGossipHopTtl });
        }
        if (configuration.maximumGossipHopTtl > SovereignMeshConsensus.ABSOLUTE_MAXIMUM_GOSSIP_HOP_TTL) {
            throw new ConsensusPreconditionError("maximumGossipHopTtl exceeds the absolute bound of 16 hops", { providedValue: configuration.maximumGossipHopTtl });
        }
        if (!Number.isInteger(configuration.deterministicReplicationFanout) || configuration.deterministicReplicationFanout <= 0) {
            throw new ConsensusPreconditionError("deterministicReplicationFanout must be a positive integer", { providedValue: configuration.deterministicReplicationFanout });
        }
        if (!Number.isInteger(configuration.maximumDeduplicationCacheEntries) || configuration.maximumDeduplicationCacheEntries <= 0) {
            throw new ConsensusPreconditionError("maximumDeduplicationCacheEntries must be a positive integer", { providedValue: configuration.maximumDeduplicationCacheEntries });
        }
        if (configuration.maximumDeduplicationCacheEntries > SovereignMeshConsensus.ABSOLUTE_MAXIMUM_DEDUPLICATION_CACHE_ENTRIES) {
            throw new ConsensusPreconditionError("maximumDeduplicationCacheEntries exceeds the absolute bound of 10000 entries", { providedValue: configuration.maximumDeduplicationCacheEntries });
        }
        if (!Number.isInteger(configuration.quorumThresholdNumerator) ||
            ...
            throw new ConsensusPreconditionError("quorum threshold must be a valid fraction in (0, 1]", {
        ...
        if (!Number.isInteger(configuration.maximumEpochDurationMillis) || configuration.maximumEpochDurationMillis <= 0) {
            throw new ConsensusPreconditionError("maximumEpochDurationMillis must be a positive integer", { providedValue: configuration.maximumEpochDurationMillis });
        }
        ...
        this.epochRecordsByNumber = new Map();
        this.registeredPeerNodes = new Map();
        this.lamportClockCounter = 0;
    }
    // --------------------------------------------------------------------------
    // Peer registration and mesh topology
    // --------------------------------------------------------------------------
    registerPeerNode(peerNodeIdentifier) {
        if (peerNodeIdentifier.publicKeyHex.length === 0) {
            throw new ConsensusPreconditionError("Peer public key hex must be non-empty", { networkEndpoint: peerNodeIdentifier.networkEndpoint });
        }
        if (peerNodeIdentifier.networkEndpoint.length === 0) {
            throw new ConsensusPreconditionError("Peer network endpoint must be non-empty", { publicKeyHex: peerNodeIdentifier.publicKeyHex });
        }
        if (peerNodeIdentifier.lamportClock < 0) {
            throw new ConsensusPreconditionError("Peer lamport clock must be non-negative", { publicKeyHex: peerNodeIdentifier.publicKeyHex, lamportClock: peerNodeIdentifier.lamportClock });
        }
    ...
    registerDirectedMeshEdge(fromPeerKeyHex, toPeerKeyHex) {
        if (!this.registeredPeerNodes.has(fromPeerKeyHex)) {
            throw new ConsensusPreconditionError("Source peer is not registered", { fromPeerKeyHex });
        }
        if (!this.registeredPeerNodes.has(toPeerKeyHex)) {
            throw new ConsensusPreconditionError("Destination peer is not registered", { toPeerKeyHex });
        }
        this.meshRoutingTable.registerDirectedEdge(fromPeerKeyHex, toPeerKeyHex);
    }
    advanceLamportClock(observedLamportClock) {
    ...
    // --------------------------------------------------------------------------
    acquirePeerLocksMonotonically(peerPublicKeyHexes) {
        for (const peerKeyHex of peerPublicKeyHexes) {
            if (!this.registeredPeerNodes.has(peerKeyHex)) {
                throw new ConsensusPreconditionError("Cannot lock an unregistered peer", { peerKeyHex });
            }
        }
        this.peerLockRegistry.acquireLocksMonotonically(peerPublicKeyHexes);
    }
    releasePeerLocksMonotonically(peerPublicKeyHexes) {
        this.peerLockRegistry.releaseLocksMonotonically(peerPublicKeyHexes);
    }
    currentlyHeldPeerLockCount() {
        return this.peerLockRegistry.currentlyHeldLockCount();
    }
    // --------------------------------------------------------------------------
    // Bounded gossip dispatch (Daf' al-Tasalsul)
    // --------------------------------------------------------------------------
    dispatchGossipEnvelope(originPeerKeyHex, payload, envelopeIdentifier) {
        if (!this.registeredPeerNodes.has(originPeerKeyHex)) {
            throw new ConsensusPreconditionError("Gossip origin peer is not registered", { originPeerKeyHex });
        }
        if (envelopeIdentifier.length === 0) {
            throw new ConsensusPreconditionError("Gossip envelope identifier must be non-empty", { originPeerKeyHex });
        }
        if (payload.payloadBytes.length === 0) {
            throw new ConsensusPreconditionError("Gossip payload bytes must be non-empty", { envelopeIdentifier });
        }
        if (payload.originPublicKeyHex !== originPeerKeyHex) {
            throw new ConsensusPreconditionError("Payload origin does not match gossip origin peer", { payloadOrigin: payload.originPublicKeyHex, gossipOrigin: originPeerKeyHex });
        }
        const initialRoutingPath = {
        ...
        const dispatchedEnvelopeIdentifiers = [];
        const suppressedDuplicateEnvelopeIdentifiers = [];
        const terminatedEnvelopeIdentifiers = [];
        const pendingEnvelopeQueue = [initialEnvelope];
        let processedEnvelopeCount = 0;
        const maximumProcessedEnvelopeCount = this.configuration.maximumGossipHopTtl *
        ...
        while (pendingEnvelopeQueue.length > 0) {
            if (processedEnvelopeCount >= maximumProcessedEnvelopeCount) {
                throw new MeshHopLimitExceededError("Gossip dispatch exceeded the deterministic processing horizon", {
            ...
            const currentPeerKeyHex = currentEnvelope.routingPath.traversedPeerKeyHexes[currentEnvelope.routingPath.traversedPeerKeyHexes.length - 1];
            if (currentPeerKeyHex === undefined) {
                throw new MeshRoutingCycleError("Routing path vector is empty during gossip dispatch", { envelopeIdentifier: currentEnvelope.envelopeIdentifier });
            }
            ...
            for (const neighborPeerKeyHex of selectedNeighborKeys) {
                if (currentEnvelope.routingPath.traversedPeerKeyHexes.includes(neighborPeerKeyHex)) {
                    throw new MeshRoutingCycleError("Gossip forwarding would revisit an already traversed peer", {
    ...
    currentDeduplicationCacheEntryCount() {
        return this.deduplicationCache.currentEntryCount();
    }
    // --------------------------------------------------------------------------
    // Epoch consensus ('Adam al-Tanaqud)
    // --------------------------------------------------------------------------
    openConsensusEpoch(epochNumber, proposedValueHashHex) {
        if (!Number.isInteger(epochNumber) || epochNumber < 0) {
            throw new ConsensusPreconditionError("Epoch number must be a non-negative integer", { providedEpochNumber: epochNumber });
        }
        if (proposedValueHashHex.length === 0) {
            throw new ConsensusPreconditionError("Proposed value hash must be non-empty", { epochNumber });
        }
        if (this.epochRecordsByNumber.has(epochNumber)) {
            throw new EpochDivergenceError("Epoch number is already registered", { epochNumber });
        }
        const epochRecord = {
            epochNumber,
    ...
    transitionEpochToVoting(epochNumber) {
        const currentRecord = this.requireEpochRecord(epochNumber);
        if (currentRecord.state !== ConsensusEpochState.PROPOSING) {
            throw new EpochDivergenceError("Epoch can only transition to VOTING from PROPOSING", { epochNumber, currentState: currentRecord.state });
        }
        const updatedRecord = {
            ...currentRecord,
    ...
    submitConsensusVoteBallot(ballot) {
        if (!Number.isInteger(ballot.epochNumber) || ballot.epochNumber < 0) {
            throw new ConsensusPreconditionError("Ballot epoch number must be a non-negative integer", { providedEpochNumber: ballot.epochNumber });
        }
        if (ballot.voterPublicKeyHex.length === 0) {
            throw new ConsensusPreconditionError("Ballot voter public key hex must be non-empty", { epochNumber: ballot.epochNumber });
        }
        if (!this.registeredPeerNodes.has(ballot.voterPublicKeyHex)) {
            throw new SignatureVerificationError("Ballot voter is not a registered peer", { voterPublicKeyHex: ballot.voterPublicKeyHex, epochNumber: ballot.epochNumber });
        }
        if (ballot.ballotSignatureHex.length === 0) {
            throw new SignatureVerificationError("Ballot signature must be non-empty", { voterPublicKeyHex: ballot.voterPublicKeyHex, epochNumber: ballot.epochNumber });
        }
        if (ballot.lamportTimestamp < 0) {
            throw new ConsensusPreconditionError("Ballot lamport timestamp must be non-negative", { voterPublicKeyHex: ballot.voterPublicKeyHex, lamportTimestamp: ballot.lamportTimestamp });
        }
        const currentRecord = this.requireEpochRecord(ballot.epochNumber);
        if (currentRecord.state !== ConsensusEpochState.VOTING) {
            throw new EpochDivergenceError("Ballots may only be submitted while the epoch is in VOTING state", { epochNumber: ballot.epochNumber, currentState: currentRecord.state });
        }
        if (ballot.proposedValueHashHex !== currentRecord.proposedValueHashHex) {
            throw new EpochDivergenceError("Ballot proposed value hash diverges from the epoch proposal", {
        ...
        for (const existingBallot of currentRecord.collectedBallots) {
            if (existingBallot.voterPublicKeyHex === ballot.voterPublicKeyHex) {
                throw new EpochDivergenceError("Duplicate ballot from the same voter within an epoch", { epochNumber: ballot.epochNumber, voterPublicKeyHex: ballot.voterPublicKeyHex });
            }
        }
    ...
    finalizeConsensusEpoch(epochNumber) {
        const currentRecord = this.requireEpochRecord(epochNumber);
        if (currentRecord.state !== ConsensusEpochState.VOTING) {
            throw new EpochDivergenceError("Epoch can only be finalized from VOTING state", { epochNumber, currentState: currentRecord.state });
        }
        const totalRegisteredPeerCount = this.registeredPeerNodes.size;
        if (totalRegisteredPeerCount === 0) {
            throw new ConsensusPreconditionError("Cannot finalize an epoch with zero registered peers", { epochNumber });
        }
        const requiredQuorumCount = Math.ceil((totalRegisteredPeerCount * this.configuration.quorumThresholdNumerator) /
            this.configuration.quorumThresholdDenominator);
        const distinctVoterKeyHexSet = new Set();
    ...
    requireEpochRecord(epochNumber) {
        const epochRecord = this.epochRecordsByNumber.get(epochNumber);
        if (!epochRecord) {
            throw new EpochDivergenceError("Epoch record not found", { epochNumber });
        }
        return epochRecord;
    }
    getLamportClock() {
...
exports.SovereignMeshConsensus = SovereignMeshConsensus;
SovereignMeshConsensus.ABSOLUTE_MAXIMUM_GOSSIP_HOP_TTL = 16;
SovereignMeshConsensus.ABSOLUTE_MAXIMUM_DEDUPLICATION_CACHE_ENTRIES = 10000;
